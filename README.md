...
### Option D: Use OpenClaude from another repo
...
openclaude() {
  if [ -n "$1" ]; then
    (cd "$1" && /home/zack/openclaude/bin/openclaude)
  else
    (/home/zack/openclaude/bin/openclaude)
  fi
}
```
...
If search stops working because the bundled `rg` binary is missing, install `ripgrep` with your OS package manager and restore the expected binary path. For example, on Ubuntu:

```bash
sudo apt install ripgrep
```

Then ensure the Linux bundle path resolves to `dist/vendor/ripgrep/x64-linux/rg`, for example by creating the directory and linking the binary:

```bash
mkdir -p /home/zack/openclaude/dist/vendor/ripgrep/x64-linux
ln -s /usr/bin/rg /home/zack/openclaude/dist/vendor/ripgrep/x64-linux/rg
```
