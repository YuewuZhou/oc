...
if [[ "${OPENCLAUDE_DEPTH:-0}" -gt 0 ]]; then
    echo "ERROR: derek.sh cannot be invoked from within a Derek or Greg subagent (OPENCLAUDE_DEPTH=${OPENCLAUDE_DEPTH}). Perform research directly using your built-in tools — do NOT invoke any skills or subagents." >&2
    exit 1
fi
